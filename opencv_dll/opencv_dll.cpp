...
{
	//return static_cast< void* > (new Recog);
	return new Recog;
	//return NULL;
}

extern "C++" DLLEXPORT void ProcessImage(Mat& TheFrame, vector<pair<int, int> > & FacePartCoords, int & ResultCode)
{
	Mat frame = TheFrame;
	ResultCode = 0;

	//-- 0. Init HOG descriptors
	HOGDescriptor hog, hog_nose, hog_mouth, hog_faceup, hog_facedown;
#ifdef _DEBUG
	InitHogDescriptor(hog,			"C:/_hog/my_photos/_eyes");
	InitHogDescriptor(hog_nose,		"C:/_hog/my_photos/_nose");
	InitHogDescriptor(hog_mouth,	"C:/_hog/my_photos/_mouth");
#else
	InitHogDescriptor(hog,			"hog/_eyes");
	InitHogDescriptor(hog_nose,		"hog/_nose");
	InitHogDescriptor(hog_mouth,	"hog/_mouth");
#endif
	//-- 1. Load the cascades
	...
	if( !face_prof_cascade.load	( face_prof_cascade_name ) )	{ printf("--(!)Error loading\n"); return; };
	...
	if(faces.size() > 0)
	{
		...
		hog.detectMultiScale		(faceROI,		eyes,	0.2,	Size(8, 8),	Size(16, 16),	1.03,	2.0);
		hog_nose.detectMultiScale	(faceROI_nose,	noses,	0.5,	Size(8, 8),	Size(16, 16),	1.03,	2.0);
		hog_mouth.detectMultiScale	(faceROI_mouth,	mouths,	0.25,	Size(8, 8),	Size(16, 16),	1.03,	2.0);
